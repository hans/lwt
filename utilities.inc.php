...
function check_update_db() {
  ...
  while ($row = mysql_fetch_row($res)) 
	...
	if (in_array($tbpref . 'archivedtexts', $tables) == FALSE) {
		...
		runsql("CREATE TABLE IF NOT EXISTS " . $tbpref . "archivedtexts ( AtID int(11) unsigned NOT NULL AUTO_INCREMENT, AtLgID int(11) unsigned NOT NULL, AtTitle varchar(200) NOT NULL, AtText text NOT NULL, AtAnnotatedText LONGTEXT NOT NULL, AtAudioURI varchar(200) DEFAULT NULL, PRIMARY KEY (AtID), KEY AtLgID (AtLgID) ) ENGINE=MyISAM DEFAULT CHARSET=utf8",'');
	}
	
	if (in_array($tbpref . 'languages', $tables) == FALSE) {
		if ($debug) echo '<p>DEBUG: rebuilding languages</p>';
		runsql("CREATE TABLE IF NOT EXISTS " . $tbpref . "languages ( LgID int(11) unsigned NOT NULL AUTO_INCREMENT, LgName varchar(40) NOT NULL, LgDict1URI varchar(200) NOT NULL, LgDict2URI varchar(200) DEFAULT NULL, LgGoogleTranslateURI varchar(200) DEFAULT NULL, LgGoogleTTSURI varchar(200) DEFAULT NULL, LgTextSize int(5) unsigned NOT NULL DEFAULT '100', LgCharacterSubstitutions varchar(500) NOT NULL, LgRegexpSplitSentences varchar(500) NOT NULL, LgExceptionsSplitSentences varchar(500) NOT NULL, LgRegexpWordCharacters varchar(500) NOT NULL, LgRemoveSpaces int(1) unsigned NOT NULL DEFAULT '0', LgSplitEachChar int(1) unsigned NOT NULL DEFAULT '0', PRIMARY KEY (LgID), UNIQUE KEY LgName (LgName) ) ENGINE=MyISAM DEFAULT CHARSET=utf8",'');
	}
	...
	if (in_array($tbpref . 'tags', $tables) == FALSE) {
		...
		runsql("CREATE TABLE IF NOT EXISTS " . $tbpref . "tags ( TgID int(11) unsigned NOT NULL AUTO_INCREMENT, TgText varchar(20) CHARACTER SET utf8 COLLATE utf8_bin NOT NULL, TgComment varchar(200) NOT NULL, PRIMARY KEY (TgID), UNIQUE KEY TgText (TgText) ) ENGINE=MyISAM DEFAULT CHARSET=utf8",'');
	}
	...
	if (in_array($tbpref . 'tags2', $tables) == FALSE) {
		...
		runsql("CREATE TABLE IF NOT EXISTS " . $tbpref . "tags2 ( T2ID int(11) unsigned NOT NULL AUTO_INCREMENT, T2Text varchar(20) CHARACTER SET utf8 COLLATE utf8_bin NOT NULL, T2Comment varchar(200) NOT NULL, PRIMARY KEY (T2ID), UNIQUE KEY T2Text (T2Text) ) ENGINE=MyISAM DEFAULT CHARSET=utf8",'');
	}
	...
	if ($count > 0) {		
		...
		while ($record = mysql_fetch_assoc($res)) {
			...
			adjust_autoincr($tbpref . 'sentences','SeID');
			adjust_autoincr($tbpref . 'textitems','TiID');
			splitText(
...
if ($len_tbpref > 0) {
	...
	for ($i=0; $i < $len_tbpref; $i++) 
		if (strpos("0123456789abcdefghijklmnopqrstuvwxyz", substr($tbpref,$i,1)) === FALSE) die('Table prefix "' . $tbpref . '" contains characters or digits other than "0123456789abcdefghijklmnopqrstuvwxyz". Please fix in "connect.inc.php".'); 
} 
